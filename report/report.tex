...
The building footprint correspondence will be evaluated using a similarity function that difference between the turning funcitons of matching building footprints in both datasets.
...
(Fan et al., 2014) further discussed how the relative overlap in building area can be used to determine building correspondence between datasets, in cases where there is not much displacement between OSM building footprints data and the reference data set. Figure 1 shows a typical comparison between the OSM and SLU datasets, and by eyesight it was determined that the displacement of features is small. The relative building overlap between the OSM and SLU datasets will be defined as folows, given the footprints of any building in the OSM set ($foot_{OSM}$) and the SLU set ($foot_{SLU}$):
...
\section{Implementation: Collision checks}
