...
It is often desirable to recreate real-world cities in 3D applications such as urban design software and video games, and the topic of 3D city reproduction comes with its own set of unique challenges, at the base of which lies procedual rendering of 3D road networks.
...
To examine this, a slice of the OSM dataset in the Stockholm area has been taken, and each road has been assigned with standard withds according to swedish regulation, after which a geometrical algorithm was applied to all road pairs to obtain the average amount of road overlap.
...
Any geodataset has a limit on its positional accuracy, that is determined by the theoretical precision of the measuring methods.
...
\begin{abstract}
Modellering av städer i 3D har länge varit ett ämne av intresse inom datorgrafik.
Ofta vill man återskapa verkliga städer i 3D-miljöer som till exempel mjukvara för stadsplanering eller datorspel.
Avbildning av städer i 3D är ett ämne som kommer med många unika utmaningar, bland vilka en central utmaning är procedurell rendrering av vägnät.

Det här projektet har undersökt hur genomförbart det är att återskapa en stad utifrån data från OpenStreetMap (OSM), och kommer särskilt fokusera på hur genomförbart det är att inkludera gångvägar i en sådan modell.
...
This project will strive to determine the feasibility of procuderally reconstructing a city road network from OSM data by using such models as have been mentioned in this section, focusing on pedestrian footpaths.
...
% TODO: Use overlap instead of collision outside of technical contexts
% TODO: Come up with a sentence that explains why errors could cause overlap. Repeat it in Introduction, research question and the abstract

Specifically in the case of rendering road networks and buildings, a common issue is that the 3D models being generated from road and building data migh overlap with each other, which can lead to strange-looking mesh intersections and inaccuracies that modelers often want to avoid.
...
In this project it was hypothised that the risk of feature collision may be higher between streets and roads designated for energy efficient mobility, such as footpaths, since they are much more organic and dense in their nature than main streets.
