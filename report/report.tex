...
\alttitle{Kvalitetiv undersökning av OpenStreetMaps gångvägsdata}
...
Feature overlap is a type of geomerty error that arises to inaccuracies in the source data, which can lead to strange-looking mesh intersections and artifacts in the final city model.
...
In this project, the accuracy was obtained by comparing coordinate points of OSM building footprints to those in the Swedish University of Agricultural Sciences (SLU) building datset, whose accuracy is known.
...
Every geodata service has an upper limit on the positional accuracy of their data, which depends primarily on the method of data acquistion that was used.
...
It was also hypothised that one can use the individual road widths and road vertex coordinates as variables when making adjustments against overlap errors.
By first detecting critical locations where feature overlap occurs and subsequently correcting this overlap by adjusting the road width and individual vertex positions within the margin of the positional accuracy, this project examined wether or not it was possible to obtain a slightly altered map with little to no overlap between footpaths and other features.
...
It was hypothised that most road overlap can be corrected by first detecting critical locations where feature overlap occurs, and subsequently adjust the footpath road widths and individual vertex positions within the margin of the OSM positional accuracy, in order to obtain a slightly altered map with little to no overlap between footpaths and other features.
...
For the road overlap study, a program has taken a slice of the OSM dataset in the Stockholm area and assigned each road with standard withds according to swedish regulation.
...
For the overlap study, see sections 3.1.2 for a method overview, 3.3 for a detailed walkthrough of the method, and 4.1.1 for the results.
...
The positional accuracy will be neccessary to carry out the investigation of the first subquestion: the rate of collisions between roads, since it will determine the maximum tolerance in distance between road points before a collision is certain to have occured.
...
For the positional accuracy study, see sections 3.1.1 for a method overview, 3.2 for a walkthrough of the method, and 4.1.2 for the results.
...
Programmatic collection of statistics has been neccessary for both the primary and secondary research questions, and this section presents a broad overview of the technical details behind the map analysis and data collection processes for both the geodata precision study and the overlap study.
...
% With internet
% TODO: Restructure research subquestions according to Chris's suggestions (see meeting log)
...
Thus it can very well be assumed that city modelling software could use the same routine for importing OSM car roads as well as OSM footpaths, without a drastic increase in feature overlap.
If it is desired to obtain a map will signifcantly less overlap than what OSM can provide due to limitations on positional accuracy, the designer could build upon their existing procedure and add algorithms for overlap detection and correction.
