...
Varje geodatamängd har en gräns på hur väl den stämmer överrens med verkligheten, som bestäms av vilka mätmedoter som har använts i datans insamling.
...
To investigate the feasibility of generating a 3D virtual city with minimal footpath overlap, the main research question will specifically be divided into two subquestions. The primary subquestion is:
...
The secondary subquestion is:

\begin{center}
    \textbf{What is the positional accuracy of OSM map features compared to a reference map?}.
\end{center}

The positional accuracy will be necessary to carry out the investigation of the second subquestion: the rate of collisions between roads, since it will determine the maximum tolerance in distance between road points before a collision is certain to have occurred.
...
The \textbf{road network} is what ties the city together as a whole, and forms the grid that divides the lots, quarters and neighborhoods, giving the city grid its unique aestethic character and defining possible building arrangements.
...
The methods used for the geodata precision study were largely based on those used by \textbf{(Fan et al., 2014)} in their study of the quality of the OSM Munich dataslice in 2014.
...
Making translation withing the map resolution will preserve the logical consistency to OSM source data, and preserve aestetic characteristics of the map, both between features and in the map as a whole
...
Provided that the corresponding building footprints are true representations of the real life building, these pairs of matching verties must match the same part of the building.
...
This will eliminate dense vertex clusters, as well as any vertices that lie in roughly straight lines, in favour of creating polygons with reduced vertex counts.
...
It was found in the feature overlap study that the individual cases of overlap could be classified into types, where all collisions of the same type could in theory be corrected by the same algoritm.
...
There could very well be additional types of collisions that are exclusive to cities with other unique city grid aestethics, which would not have been considered by this particular project.
...
Studies acroos many different areas have been done before that required very different knowledge about a city's geometry, e.g. for the purposes of modeling heat development
\textbf{(Nakata-Osaki, Souza and Rodrigues, 2018)}
or reducing noise
\textbf{(Joon, Seo and Byung, 2011)}.

According to \textbf{(Stojanovski, 2019)}, software that strives to aid in the urban development process is commonly not sophisticated enough to fully represent the needs and workspaces of urban planners in practise.
...
The case-by-case nature of urban development projects also means that it is unlikely that a single piece of software can cater to the very differing needs of data and data structures for variuos projects.
