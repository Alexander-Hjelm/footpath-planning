...
It was  that most road overlap can be corrected by first detecting critical locations where feature overlap occurs, and subsequently adjust the footpath road widths and individual vertex positions within the margin of the OSM positional accuracy, in order to obtain a slightly altered map with little to no overlap between footpaths and other features.
...
The positional accuracy will be necessary to carry out the investigation of the first subquestion: the rate of collisions between roads, since it will determine the maximum tolerance in distance between road points before a collision is certain to have occurred.
...
Attribute accuracy is the measure of how complete attribute information is in a dataset.
How much attribute data is missing and how much of it does not conform with reality?
In general, attribute accuracy is hard to quantify.
In this case however, it might be interesting to compare attributes between the OSM map and the SLU reference map that was used in this project, as both use encoded metadata on a per feature basis, and matching features can be easily identified.
...
Additionally, a number of auxiliary calculations were needed to estimate the error boundary of the positional accuracy.
...
To avoid this effect, key points were extracted using the Douglas-Peucker algorithm \textbf{(Douglas and Peucker, 1973)}, to create a simplified footprint with less points that still retain information about the rough features of the detailed footprint.
...
After visual inspection, it was determined that footpath all collision occurrences can be almost exclusively classified into one of five categories.
...
As has already been established in the analysis of the footpath overlap study, primary conclusion of the footpath overlap study is that, in terms of feature overlap, footpaths were not significantly worse than other road categories.
...
If it is desired to obtain a map will significantly less overlap than what OSM can provide due to limitations on positional accuracy, the designer could build upon their existing procedure and add algorithms for overlap detection and correction.
...
Method exist for how to make a city model conform to an underlying terrain, however.
