...
Virtual cities has in recent years been a popular research topic in the field of computer visualization, according to \textbf{(Vanegas et al, 2010)}.
...
% TODO: Mention that positional accuracy will be focused on, and the implications of not fixing errors when converting to a 3D model
...
However, \textbf{(Biljecki, Ledoux and Stoter, 2016 (1))} note that there are coming more and more sophisticated methods for automatically correcting the most common errors.
Geodata validation and correction is only an emerging field of research.
...
For the primary research question: the collision study, a program has taken an OSM dataset of the Stockholm area and assigned each road with standard withds according to swedish regulation.
...
Lineage is defined by (\textbf{Van Oort, 2006}) as the historical aspect of the dataset: how it has been obtained, with what instruments and how it has evolved between the moment it was obtained and the moment it was queried.
...
Attributes are small flags of metadata that are included with geodata features, letting the user know for example the type of a road, how many files it has, if the base is asphalt or gravel, or how many stories a building has.
Attribute accurcay is the meausere of how complete attribute information is in a dataset.
How much attribute data is missing and how much of it does not comform with reality?
In general, attribute accuracy is hard to quantify, but might be interesting, as both the OSM map and the referece map used in this project use encoded metadata on a per feature basis.
...
This chapter will dive into the state-of-the-art use of city reconstruction methods within the research field of urban design.
This chapter will present the motivations for the digitization of urban planning, as well as a brief history of the field of OpenStreetMap data qualification, and present the research papers whose methods are at the foundation of this project.
Furthermore, it will present the evaluation metrics that this project served to obtain, and how they relate to the eventual collision analysis.
...
At the very base of this process is the procedural layout of road networks in 3D, and so far there have been a lot of research in modelling of 3D road networks.
...
Procedural modeling of road networks is of interest to the fields of urban planning and urban architecture.
...
Procedural and AI solutions that aid the arcitect in a form of human-machine symbiosis.
...
When assessing the completeness of geodata, a very common scale is Van Oorts criteria for evaluating the quality of geographical information, which at the time of publication received attention from surveyors and cartographers (\textbf{(Van Oort, 2006)}).
Since then a number of publications have examined slices of geodata from van Oort's criteria.
...
Other notable studies about OSM data accuracy were made around the same time by in Germany by \textbf{Zielstra and Zipf, 2010} and \textbf{Neis et al, 2012}, as well as in France by \textbf{Girres and Touya, 2010}.
...
Adding such techniques for content creation into the workflow of city reconstruction will increase the level of detail that is possible, while sacrificing accuracy to the real-world.
\textbf{(Müller et al, 2006)} present how to accomplish this using Shape Grammar: a method developed by \textbf{Stiny, 1975}, for automatically and iteratively constructing 3D models based in a hierarchical set of rules.
Shape grammar was originally developed from a method known as L-systems: a system for model description which was originally used to generate plants for 3D animation \textbf{Parish and Müller, 2001}.
Shape grammar works by starting with a crude volumetric model of a building and iteratively remodeling it from a set of shape grammars, a procedural model can structure the façade, add texture and add details for windows, doors and ornaments, and so on.
...
Space syntax, developed by \textbf{Hillier, 1997}, is a set of theories that analyze complex, large-scale spaces such as cities, and tries to explain human behavior from a spatial point of view. The central idea is that spatial structures sucha as building patterns both reflect and create the patterns for human interaction with their surroundings.
Dividing spacial structures into different levels of observation, and identifying different units of space and the pattern in which they connect to each other.
Space syntax is an analytical method that relies on the availablity of city maps. Has since been developed into an extensive research fields on its own.
Space syntax is commonly used in the analysis of procedural generation models of urban environments
\textbf{Parish and Müller, 2001},
and has been widelry used in the analysis of pedestrian flows and wayfinding
\textbf{(Peponis, Zimring and Choi, 1990)}.
The idea of using space syntax to divide spatial structures into different levels of observation is frequently used in the research field of urban design and planning, to gain an understanding how space is utilized and connected at different levels of usage (one can for example analyze a city on an interior-by-interior basis, or use city blocks and streets as the least unit of space)
...
%TODO: summarize these (DP with reference to DP and Fan)
Douglas-Peucker algorithm for polygon reduction. The full procedure for matching points will be presented in section ***.
The building footprint correspondence will be evaluated using a similarity function that depends on the difference between the turning functions of matching building footprints in both datasets.
...
and used by \textbf{Fan et al, 2014} to obtain a measure of the shape accuracy of building footprints.

\subsection{Level of detail}

Another crucial concept in city modelling is the Level Of Detail (LOD) aspect.
\textbf{(Biljecki, Ledoux and Stoter, 2016 (1))} provide an extensive overview of the concept of LOD in GIS mapping.
They provide the GityGML definition of LOD as a series of different 3D representation of the same data, at different detail levels, and the LOD chosen for a particulary application suggests how the data has been aqquired and modelled.
...
%TODO: Summarize both sources by Biljecki
\textbf{Biljecki, Ledoux and Stoter, 2016 (1)}
...
\textbf{(Rutzinger, Rottensteiner and Pfeifer, 2009)}, 30% area overlap
...
The driving force behind the availablity of VGI is, according to
\textbf{Goetz and Zipf, 2012},
the availablity of high resolution satellite imagery to private individuals. They claim that particularily the imagery from Bing Maps, which launched in 2010, led to an increase in building information in OSM, and the positional error is largely due to the limited resolution in such satellite imagery.
...
% After Chris approves
% TODO: Update section references
% TODO: Spell check
