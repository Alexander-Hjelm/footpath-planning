...
To avoid this effect, key points are extracted using the Douglas-Peucker algorithm \cite{douglas-73}, to create a simplifed footprint with less points that still retain information about the rough features of the detailed footprint.
...
The same methods can be applied to import building footprint data from OSM or some, and the same metrics can be applied to assess the data accuracy.
When we have the fundamental
we can proceed with building modeling (Parish Müller, procedural building modeling). Other challenges include infering or guessing building height. () have presented a method for inferring building height from the building footprint alone, with surprising accuracy.
