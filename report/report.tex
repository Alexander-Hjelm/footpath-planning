...
Description of the field of city reproduction (Parish Müller, procedural modeling of ciites).
...
Applications:
    ...
    (Parish Müller: Procedural modelin of cities)

Description of different levels of subdivision. What levels of subdivision are we looking at? Picture, ref to Todor's paper.
Viewpoints can range from modelling individual roads (find reference) and building facades (todor) to looking at entire city parcels and how they connect with each other (find reference).
...
(Parish Müller, Procedural modeling of buildings)

At the base of this process is the procedural layout of road networks in 3D.
(Parish Müller, Procedural modeling of cities)

So far there have been a lot of research in modelling of 3D road networks, but these projects have often focused on the road networks designated for cars and public transit, due to environmental concerns (Todors paper)
...
%TODO: (Parish Müller: Procedural modelin of cities)
...
% Thursday:
% TODO: Fix citings like they appear in the example thesis
...
% TODO: Write Collision study implementation section
    % TODO: Collision study implementation, disposition
...
The same methods can be applied to import building footprint data from OSM or some, and the same metrics can be applied to assess the data accuracy. (Fan et al)
When we have the fundamental procedure for accuratly importing building footprints without compromising the data integritey, we can proceed with building modeling (Parish Müller, procedural building modeling). Other challenges include infering or guessing building height. () have presented a method for inferring building height from the building footprint alone, with surprising accuracy.
...
We may also chose what LOD class(es) to use depending on data availablity and application. (find reference)
