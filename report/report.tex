...
\begin{figure}[H]
    ...
    \caption{Real-world cities are complex collections of buildings, blocks and road networks that interconnect them all. Creating 3D virtual cities using methods of procedual generation is an emerging research field that comes with its own set of challenges.}
...
Virtual city reproduction has a growing number of applications, including visualization, urban planning and entertainment in the form of video games and 3D animation.
Traditionally, modelling virtual cities is a labour-intensive and time-consuming task.
In recent years there has been major research work into the field of procedural content generation, meaning automatic or semi automatic generation of 3D models
Such methods can indeed be used in the field of virtual city reproduction to reduce the labour required to reproduce accurate 3D representations of cities.
But the field of virtual city reproduction comes with its own host of challenges.
...
Procedural models for city reproduction instead rely on programmatic solutions for converting map data to 3D road and building models, and composing them to a 3D world that the designer may continue to fine-tweak through interfaces fo interactive editing.
...
Currently, when modeling
3D GIS datasets without errors are rare. While previously \textbf{(Biljecki et al, 2016)} note that there are coming more and more sophisticated methods for automatically correcting the most common errors.
Among the most common errors: Missing or invalid geometry, invalid semantics.
GIS validation har only been a topis of research lately, and many such datasets were usually obtained years ago, and creating error-free datasets was neither possible due to the lack of validation techniques, nor a high priority.

The idea of gathering geodata from OpenStreetMap (OSM), where the temporal aspect of data accuracy is perhaps mitigated by the crowd contribution nature.
There have already been a few services (*** Name a few services + links)
but such services are usually limited in the type of data that they can provide. Usually they focus on obtaining either road networks or buildings separately, and in the case of road networks they usually focus on car road networks (why? Driving games?***), and roads designated for energy efficient mobility are left out.

This project assesses the feasibility of reconstructing a city from OSM road and building data, and will focus on the feasibility of including pedestrian footpaths, by focusing on OSM road and building data in the Stockholm metropolitan area.
...
% Pictures
% Introduction
    % TODO: Figure showing the unique characteristics of footpaths vs other road types (OSM Screeshot)
    % TODO: Virtual city picture
% Scietific study
    % TODO: Find papers and topics for the scientific study (building heights, road rendering, terrain (Procedural roads))
    % TODO: Scientific study, disposition
% TODO: Edge distance algorithm, algorithm

% Monday
% TODO: Write introduction section
% TODO: Write Collision study implementation section
% TODO: Write Scientific study
% Collision study implementation
    % TODO: Figure detailing the steps in the point-matching algorithm
    % TODO: Edge distance algorithm, picture
...
When we have the fundamental procedure for accuratly importing building footprints without compromising the data integritey, we can proceed with building modeling. \textbf{(Müller et al, 2006)}

Terrain elevation. Elevation data is available from other sources, but not by OSM.
\textbf{(Galin et al, 2010)} present a geometrical model for placing a 3d road asset onto a non-flat terrain surface.

Other challenges include infering or guessing building height.
